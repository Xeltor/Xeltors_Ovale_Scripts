...
AddFunction FireActiveTalentsShortCdActions
{
    unless { BuffExpires(combustion_buff) or BuffPresent(combustion_buff) and Charges(fire_blast) < 1 and Charges(phoenixs_flames) < 1 } and Spell(blast_wave)
    {
        ...
        if SpellCooldown(combustion) > 40 or SpellCooldown(combustion) > target.TimeToDie() or BuffPresent(rune_of_power_buff) or FIXME_firestarter.active Spell(meteor)
...
{
    { BuffExpires(combustion_buff) or BuffPresent(combustion_buff) and Charges(fire_blast) < 1 and Charges(phoenixs_flames) < 1 } and Spell(blast_wave) or { SpellCooldown(combustion) > 40 or SpellCooldown(combustion) > target.TimeToDie() or BuffPresent(rune_of_power_buff) or FIXME_firestarter.active } and Spell(meteor) or { SpellCooldown(combustion) < CastTime(cinderstorm) and { BuffPresent(rune_of_power_buff) or not Talent(rune_on_power_talent) } or SpellCooldown(combustion) > 10 * { 100 / { 100 + SpellHaste() } } and not BuffPresent(combustion_buff) } and Spell(cinderstorm) or { HasEquippedItem(132863) or Talent(alexstraszas_fury_talent) and BuffExpires(hot_streak_buff) } and Spell(dragons_breath) or Enemies(tagged=1) > 1 and BuffExpires(combustion_buff) and Spell(living_bomb)
}
...
AddFunction FireRopPhaseMainActions
{
    ...
    unless FireActiveTalentsMainPostConditions()
    {
        ...
        if not PreviousGCDSpell(phoenixs_flames) and Charges(phoenixs_flames count=0) > 2.7 and FIXME_firestarter.active Spell(phoenixs_flames)
...
AddFunction FireRopPhaseShortCdActions
{
    ...
    unless { Talent(flame_patch_talent) and Enemies(tagged=1) > 1 or Enemies(tagged=1) > 3 } and BuffPresent(hot_streak_buff) and Spell(flamestrike) or BuffPresent(hot_streak_buff) and Spell(pyroblast)
    {
        ...
        unless FireActiveTalentsShortCdPostConditions() or BuffPresent(kaelthas_ultimate_ability_buff) and ExecuteTime(pyroblast) < BuffRemaining(kaelthas_ultimate_ability_buff) and Spell(pyroblast)
        {
            ...
            if not PreviousOffGCDSpell(fire_blast) and BuffPresent(heating_up_buff) and FIXME_firestarter.active and Charges(fire_blast count=0) > 1.7 Spell(fire_blast)

            unless not PreviousGCDSpell(phoenixs_flames) and Charges(phoenixs_flames count=0) > 2.7 and FIXME_firestarter.active and Spell(phoenixs_flames)
            {
                #fire_blast,if=!prev_off_gcd.fire_blast&!firestarter.active
                if not PreviousOffGCDSpell(fire_blast) and not FIXME_firestarter.active Spell(fire_blast)
...
{
    { Talent(flame_patch_talent) and Enemies(tagged=1) > 1 or Enemies(tagged=1) > 3 } and BuffPresent(hot_streak_buff) and Spell(flamestrike) or BuffPresent(hot_streak_buff) and Spell(pyroblast) or FireActiveTalentsShortCdPostConditions() or BuffPresent(kaelthas_ultimate_ability_buff) and ExecuteTime(pyroblast) < BuffRemaining(kaelthas_ultimate_ability_buff) and Spell(pyroblast) or not PreviousGCDSpell(phoenixs_flames) and Charges(phoenixs_flames count=0) > 2.7 and FIXME_firestarter.active and Spell(phoenixs_flames) or not PreviousGCDSpell(phoenixs_flames) and Spell(phoenixs_flames) or target.HealthPercent() <= 30 and HasEquippedItem(132454) and Spell(scorch) or { Talent(flame_patch_talent) and Enemies(tagged=1) > 2 or Enemies(tagged=1) > 5 } and Spell(flamestrike) or Spell(fireball)
}
...
AddFunction FireStandardRotationMainActions
{
    ...
    if BuffPresent(hot_streak_buff) and FIXME_firestarter.active and not Talent(rune_of_power_talent) Spell(pyroblast)
