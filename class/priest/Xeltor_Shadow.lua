...
AddFunction ShadowCleaveShortCdActions
{
 unless Spell(void_eruption)
 {
  ...
  unless not target.DebuffPresent(vampiric_touch_debuff) and AzeriteTraitRank(thought_harvester_trait) >= 1 and Spell(vampiric_touch) or BuffPresent(harvested_thoughts_buff) and Spell(mind_sear)
  {
   #void_bolt
   Spell(void_bolt)
   #purifying_blast
...
AddFunction ShadowCleaveCdActions
{
 unless Spell(void_eruption) or BuffExpires(voidform_shadow) and Spell(dark_ascension) or not target.DebuffPresent(vampiric_touch_debuff) and AzeriteTraitRank(thought_harvester_trait) >= 1 and Spell(vampiric_touch) or BuffPresent(harvested_thoughts_buff) and Spell(mind_sear)
 {
  ...
  unless Spell(purifying_blast) or Spell(the_unbound_force) or Spell(concentrated_flame_essence) or Spell(ripple_in_space_essence) or Spell(worldvein_resonance_essence)
  {
