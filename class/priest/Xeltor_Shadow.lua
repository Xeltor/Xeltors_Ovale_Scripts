...
do
	...
	local desc = "[Xel][7.2.5] Priest: Shadow"
	local code = [[
...
AddFunction cd_time
{
    10 + { 2 - 2 * TalentPoints(mindbender_talent) * ArmorSetBonus(T20 4) } * ArmorSetBonus(T19 2) + { 3 - 3 * TalentPoints(mindbender_talent) * ArmorSetBonus(T20 4) } * HasEquippedItem(mangazas_madness) + { 6 + 5 * TalentPoints(mindbender_talent) } * ArmorSetBonus(T20 4) + 2 * ArtifactTraitRank(lash_of_insanity)
}

AddFunction s2msetup_time
{
    if Talent(surrender_to_madness_talent) 0.8 * { 83 + { 20 + 20 * TalentPoints(fortress_of_the_mind_talent) } * ArmorSetBonus(T20 4) - 5 * TalentPoints(sanlayn_talent) + { 30 + 42 * { Enemies(tagged=1) > 1 } + 10 * TalentPoints(lingering_insanity_talent) } * ArmorSetBonus(T21 4) * TalentPoints(auspicious_spirits_talent) + { 33 - 13 * ArmorSetBonus(T20 4) } * TalentPoints(reaper_of_souls_talent) + ArmorSetBonus(T19 2) * 4 + 8 * HasEquippedItem(mangazas_madness) + SpellHaste() * 10 * { 1 + 0.7 * ArmorSetBonus(T20 4) } * { 2 + 0.8 * ArmorSetBonus(T19 2) + 1 * TalentPoints(reaper_of_souls_talent) + 2 * ArtifactTraitRank(mass_hysteria) - 1 * TalentPoints(sanlayn_talent) } }
}

AddFunction dot_vt_dpgcd
{
    71 * 1.2 * { 1 + 0.2 * TalentPoints(sanlayn_talent) } * { 1 + 0.05 * ArtifactTraitRank(touch_of_darkness) } * { 1 + 0.2 + MasteryRating() / 16000 } * 0.5
}

AddFunction dot_swp_dpgcd
{
    38 * 1.2 * { 1 + 0.06 * ArtifactTraitRank(to_the_pain) } * { 1 + 0.2 + MasteryRating() / 16000 } * 0.75
}
...
{
    80 * { 1 + 0.05 * ArtifactTraitRank(void_corruption) }
}
...
AddFunction ShadowMainMainActions
{
    ...
    if { Enemies(tagged=1) <= 4 or Talent(reaper_of_souls_talent) and Enemies(tagged=1) <= 2 } and SpellCharges(shadow_word_death) == 2 and Insanity() <= 85 - 15 * TalentPoints(reaper_of_souls_talent) Spell(shadow_word_death)
...
{
    Speed() > 0 and Talent(misery_talent) and target.DebuffRemaining(shadow_word_pain_debuff) < GCD() and Spell(shadow_word_pain) or Talent(misery_talent) and { target.DebuffRemaining(vampiric_touch_debuff) < 3 * GCD() or target.DebuffRemaining(shadow_word_pain_debuff) < 3 * GCD() } and Spell(vampiric_touch) or not Talent(misery_talent) and target.DebuffRemaining(shadow_word_pain_debuff) < { 3 + 4 / 3 } * GCD() and Spell(shadow_word_pain) or not Talent(misery_talent) and target.DebuffRemaining(vampiric_touch_debuff) < { 4 + 4 / 3 } * GCD() and Spell(vampiric_touch) or Spell(void_eruption) or { Enemies(tagged=1) <= 4 or Talent(reaper_of_souls_talent) and Enemies(tagged=1) <= 2 } and SpellCharges(shadow_word_death) == 2 and Insanity() <= 85 - 15 * TalentPoints(reaper_of_souls_talent) and Spell(shadow_word_death) or Enemies(tagged=1) <= 4 and Talent(legacy_of_the_void_talent) and { Insanity() <= 81 or Insanity() <= 75.2 and Talent(fortress_of_the_mind_talent) } and Spell(mind_blast) or { Enemies(tagged=1) <= 4 and not Talent(legacy_of_the_void_talent) or Insanity() <= 96 or Insanity() <= 95.2 and Talent(fortress_of_the_mind_talent) } and Spell(mind_blast) or not Talent(misery_talent) and not target.DebuffPresent(shadow_word_pain_debuff) and target.TimeToDie() > 10 and Enemies(tagged=1) < 5 and { Talent(auspicious_spirits_talent) or Talent(shadowy_insight_talent) } and Spell(shadow_word_pain) or Enemies(tagged=1) > 1 and not Talent(misery_talent) and not target.DebuffPresent(vampiric_touch_debuff) and dot_vt_dpgcd() * target.TimeToDie() / { GCD() * { 156 + sear_dpgcd() * { Enemies(tagged=1) - 1 } } } > 1 and Spell(vampiric_touch) or Enemies(tagged=1) > 1 and not Talent(misery_talent) and not target.DebuffPresent(shadow_word_pain_debuff) and dot_swp_dpgcd() * target.TimeToDie() / { GCD() * { 118 + sear_dpgcd() * { Enemies(tagged=1) - 1 } } } > 1 and Spell(shadow_word_pain) or Talent(shadow_word_void_talent) and Insanity() <= 75 - 10 * TalentPoints(legacy_of_the_void_talent) and Spell(shadow_word_void) or Spell(mind_flay) or Spell(shadow_word_pain)
}
...
{
    Speed() > 0 and Talent(misery_talent) and target.DebuffRemaining(shadow_word_pain_debuff) < GCD() and Spell(shadow_word_pain) or Talent(misery_talent) and { target.DebuffRemaining(vampiric_touch_debuff) < 3 * GCD() or target.DebuffRemaining(shadow_word_pain_debuff) < 3 * GCD() } and Spell(vampiric_touch) or not Talent(misery_talent) and target.DebuffRemaining(shadow_word_pain_debuff) < { 3 + 4 / 3 } * GCD() and Spell(shadow_word_pain) or not Talent(misery_talent) and target.DebuffRemaining(vampiric_touch_debuff) < { 4 + 4 / 3 } * GCD() and Spell(vampiric_touch) or Spell(void_eruption) or Talent(shadow_crash_talent) and Spell(shadow_crash) or { Enemies(tagged=1) <= 4 or Talent(reaper_of_souls_talent) and Enemies(tagged=1) <= 2 } and SpellCharges(shadow_word_death) == 2 and Insanity() <= 85 - 15 * TalentPoints(reaper_of_souls_talent) and Spell(shadow_word_death) or Enemies(tagged=1) <= 4 and Talent(legacy_of_the_void_talent) and { Insanity() <= 81 or Insanity() <= 75.2 and Talent(fortress_of_the_mind_talent) } and Spell(mind_blast) or { Enemies(tagged=1) <= 4 and not Talent(legacy_of_the_void_talent) or Insanity() <= 96 or Insanity() <= 95.2 and Talent(fortress_of_the_mind_talent) } and Spell(mind_blast) or not Talent(misery_talent) and not target.DebuffPresent(shadow_word_pain_debuff) and target.TimeToDie() > 10 and Enemies(tagged=1) < 5 and { Talent(auspicious_spirits_talent) or Talent(shadowy_insight_talent) } and Spell(shadow_word_pain) or Enemies(tagged=1) > 1 and not Talent(misery_talent) and not target.DebuffPresent(vampiric_touch_debuff) and dot_vt_dpgcd() * target.TimeToDie() / { GCD() * { 156 + sear_dpgcd() * { Enemies(tagged=1) - 1 } } } > 1 and Spell(vampiric_touch) or Enemies(tagged=1) > 1 and not Talent(misery_talent) and not target.DebuffPresent(shadow_word_pain_debuff) and dot_swp_dpgcd() * target.TimeToDie() / { GCD() * { 118 + sear_dpgcd() * { Enemies(tagged=1) - 1 } } } > 1 and Spell(shadow_word_pain) or Talent(shadow_word_void_talent) and Insanity() <= 75 - 10 * TalentPoints(legacy_of_the_void_talent) and Spell(shadow_word_void) or Spell(mind_flay) or Spell(shadow_word_pain)
}
