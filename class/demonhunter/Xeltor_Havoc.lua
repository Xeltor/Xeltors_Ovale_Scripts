...
AddFunction HavocCooldownCdActions
{
 ...
 if Talent(demonic_talent) and BuffPresent(metamorphosis_havoc_buff) and IsBossFight() Spell(metamorphosis_havoc)
...
AddFunction HavocDemonicMainActions
{
 ...
 # if not Talent(momentum_talent) and Talent(demon_blades_talent) and not SpellCooldown(eye_beam) == 0 and { Charges(fel_rush) == 2 or 600 > 10 and 600 > 10 } and CheckBoxOn(opt_fel_rush) Spell(fel_rush)
 #demons_bite
 Spell(demons_bite)
...
AddFunction HavocNormalMainActions
{
 ...
 # if Charges(fel_rush) == 2 and not Talent(momentum_talent) and not Talent(fel_mastery_talent) and not BuffPresent(metamorphosis_havoc_buff) and Talent(demon_blades_talent) and CheckBoxOn(opt_fel_rush) Spell(fel_rush)
 #fel_eruption
 Spell(fel_eruption)
...
AddFunction HavocPrecombatCdActions
{
 ...
 if not { Talent(demon_reborn_talent) and { Talent(demonic_talent) or ArmorSetBonus(T21 4) } } and IsBossFight() Spell(metamorphosis_havoc)
}
