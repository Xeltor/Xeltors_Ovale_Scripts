...
do
	...
	local desc = "[Xel][7.1.5] Monk: Windwalker"
	local code = [[
...
AddFunction WindwalkerSefCdActions
{
	unless not PreviousGCDSpell(tiger_palm) and Energy() == MaxEnergy() and Chi() < 1 and Spell(tiger_palm)
	{
		...
		unless WindwalkerCdCdPostConditions()
		{
			...
			if not BuffPresent(storm_earth_and_fire_buff) and not BuffPresent(storm_earth_and_fire_buff) Spell(storm_earth_and_fire)
			#call_action_list,name=st
			WindwalkerStCdActions()
		}
	}
...
AddFunction WindwalkerSerenityMainActions
{
	...
	unless WindwalkerCdMainPostConditions()
	{
		...
		if not PreviousGCDSpell(blackout_kick) and { PreviousGCDSpell(strike_of_the_windlord) or PreviousGCDSpell(fists_of_fury) } and Enemies(tagged=1) < 2 or SpellCount(spinning_crane_kick) >= 2 Spell(blackout_kick)
...
{
	not PreviousGCDSpell(tiger_palm) and Energy() == MaxEnergy() and Chi() < 1 and not BuffPresent(serenity_buff) and Spell(tiger_palm) or WindwalkerCdShortCdPostConditions() or Enemies(tagged=1) < 3 and Spell(rising_sun_kick) or Spell(strike_of_the_windlord) or { not PreviousGCDSpell(blackout_kick) and { PreviousGCDSpell(strike_of_the_windlord) or PreviousGCDSpell(fists_of_fury) } and Enemies(tagged=1) < 2 or SpellCount(spinning_crane_kick) >= 2 } and Spell(blackout_kick) or HasEquippedItem(drinking_horn_cover) and BuffRemaining(pressure_point_buff) <= 2 and ArmorSetBonus(T20 4) and { SpellCooldown(rising_sun_kick) > 1 or Enemies(tagged=1) > 1 } and Spell(fists_of_fury) or { not HasEquippedItem(drinking_horn_cover) or BuffPresent(burst_haste_buff any=1) or BuffRemaining(serenity_buff) < 1 } and { SpellCooldown(rising_sun_kick) > 1 or Enemies(tagged=1) > 1 } and Spell(fists_of_fury) or Enemies(tagged=1) >= 3 and not PreviousGCDSpell(spinning_crane_kick) and Spell(spinning_crane_kick) or Enemies(tagged=1) >= 3 and Spell(rising_sun_kick) or not PreviousGCDSpell(spinning_crane_kick) and Spell(spinning_crane_kick) or not PreviousGCDSpell(blackout_kick) and Spell(blackout_kick) or not PreviousGCDSpell(rushing_jade_wind) and Spell(rushing_jade_wind)
}
...
{
	not PreviousGCDSpell(tiger_palm) and Energy() == MaxEnergy() and Chi() < 1 and not BuffPresent(serenity_buff) and Spell(tiger_palm) or WindwalkerCdCdPostConditions() or Enemies(tagged=1) < 3 and Spell(rising_sun_kick) or Spell(strike_of_the_windlord) or { not PreviousGCDSpell(blackout_kick) and { PreviousGCDSpell(strike_of_the_windlord) or PreviousGCDSpell(fists_of_fury) } and Enemies(tagged=1) < 2 or SpellCount(spinning_crane_kick) >= 2 } and Spell(blackout_kick) or HasEquippedItem(drinking_horn_cover) and BuffRemaining(pressure_point_buff) <= 2 and ArmorSetBonus(T20 4) and { SpellCooldown(rising_sun_kick) > 1 or Enemies(tagged=1) > 1 } and Spell(fists_of_fury) or { not HasEquippedItem(drinking_horn_cover) or BuffPresent(burst_haste_buff any=1) or BuffRemaining(serenity_buff) < 1 } and { SpellCooldown(rising_sun_kick) > 1 or Enemies(tagged=1) > 1 } and Spell(fists_of_fury) or Enemies(tagged=1) >= 3 and not PreviousGCDSpell(spinning_crane_kick) and Spell(spinning_crane_kick) or Enemies(tagged=1) >= 3 and Spell(rising_sun_kick) or not PreviousGCDSpell(spinning_crane_kick) and Spell(spinning_crane_kick) or not PreviousGCDSpell(blackout_kick) and Spell(blackout_kick) or not PreviousGCDSpell(rushing_jade_wind) and Spell(rushing_jade_wind)
}
...
AddFunction WindwalkerStMainActions
{
	...
	unless WindwalkerCdMainPostConditions()
	{
		...
		if { Enemies(tagged=1) >= 3 or SpellCount(spinning_crane_kick) >= 3 } and not PreviousGCDSpell(spinning_crane_kick) Spell(spinning_crane_kick)
