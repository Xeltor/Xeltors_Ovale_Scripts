...
do
	local name = "hooves_guardian_73"
	local desc = "[Hooves][7.3] Druid: Guardian"
	local code = [[
...
AddFunction GuardianDefaultMainActions
{
	...
	if IncomingDamage(5) >= MaxHealth() * 0.35 and not BuffPresent(frenzied_regeneration_buff) Spell(frenzied_regeneration)
	if ((IncomingDamage(5) / 2 <= HealthMissing()) and (IncomingDamage(5) / 2 > MaxHealth() * 0.1) and SpellCharges(frenzied_regeneration) >= 2) and not BuffPresent(frenzied_regeneration_buff) Spell(frenzied_regeneration)
	if ( Health() <= MaxHealth() * 0.5  and not BuffPresent(frenzied_regeneration_buff))Spell(frenzied_regeneration)
...
AddFunction GuardianDefaultShortCdActions
{
	...
	#rage_of_the_sleeper
	 if (CheckBoxOn(rots)) spell(rage_of_the_sleeper)
	#lunar_beam
	#Spell(lunar_beam)
