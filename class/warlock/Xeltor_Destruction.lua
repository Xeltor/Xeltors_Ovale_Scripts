...
do
	...
	local desc = "[Xel][7.2.5] Warlock: Destruction"
	local code = [[
...
AddFunction DestructionDefaultMainActions
{
    ...
    if Enemies(tagged=1) == 2 and Talent(roaring_blaze_talent) and not SpellCooldown(havoc) > 0 and target.DebuffRemaining(immolate_debuff) <= BaseDuration(active_havoc_buff) Spell(immolate)
    ...
    if Enemies(tagged=1) < 4 and BuffRemaining(active_havoc_buff) > CastTime(chaos_bolt) Spell(chaos_bolt)
    ...
    if target.DebuffRemaining(immolate_debuff) > CastTime(channel_demonfire) and { Enemies(tagged=1) == 1 or BuffRemaining(active_havoc_buff) < CastTime(chaos_bolt) } Spell(channel_demonfire)
    ...
    if Enemies(tagged=1) < 3 and target.TimeToDie() <= 10 Spell(chaos_bolt)
    ...
    if Enemies(tagged=1) < 3 and { SpellCooldown(havoc) > 12 and SpellCooldown(havoc) > 0 or Enemies(tagged=1) == 1 or SoulShards() >= 5 - Enemies(tagged=1) * 0.5 } and { SoulShards() >= 5 - Enemies(tagged=1) * 0.5 or BuffRemaining(soul_harvest_buff) > CastTime(chaos_bolt) or BuffRemaining(concordance_of_the_legionfall_buff) > CastTime(chaos_bolt) } Spell(chaos_bolt)
    ...
    if Enemies(tagged=1) < 3 and { SpellCooldown(havoc) > 12 and SpellCooldown(havoc) > 0 or Enemies(tagged=1) == 1 or SoulShards() >= 5 - Enemies(tagged=1) * 0.5 } and { BuffPresent(trinket_stacking_proc_mastery_buff) and BuffRemaining(trinket_stacking_proc_mastery_buff) > CastTime(chaos_bolt) or BuffPresent(trinket_stacking_proc_crit_buff) and BuffRemaining(trinket_stacking_proc_crit_buff) > CastTime(chaos_bolt) or BuffPresent(trinket_stacking_proc_versatility_buff) and BuffRemaining(trinket_stacking_proc_versatility_buff) > CastTime(chaos_bolt) or BuffPresent(trinket_stacking_proc_intellect_buff) and BuffRemaining(trinket_stacking_proc_intellect_buff) > CastTime(chaos_bolt) or BuffPresent(trinket_stacking_proc_spell_power_buff) and BuffRemaining(trinket_stacking_proc_spell_power_buff) > CastTime(chaos_bolt) } Spell(chaos_bolt)
    #shadowburn
    Spell(shadowburn)
...
{
    unless Enemies(tagged=1) == 2 and Talent(roaring_blaze_talent) and not SpellCooldown(havoc) > 0 and target.DebuffRemaining(immolate_debuff) <= BaseDuration(active_havoc_buff) and Spell(immolate)
    {
        ...
        unless { Enemies(tagged=1) < 5 or not Talent(fire_and_brimstone_talent) } and target.DebuffRemaining(immolate_debuff) <= target.TickTime(immolate_debuff) and Spell(immolate) or { Enemies(tagged=1) < 5 or not Talent(fire_and_brimstone_talent) } and { not Talent(cataclysm_talent) or SpellCooldown(cataclysm) >= CastTime(immolate) * Enemies(tagged=1) } and Enemies(tagged=1) > 1 and target.DebuffRemaining(immolate_debuff) <= target.TickTime(immolate_debuff) and { not Talent(roaring_blaze_talent) or not target.DebuffPresent(roaring_blaze_debuff) and Charges(conflagrate) < 2 + ArmorSetBonus(T19 4) } and Spell(immolate) or Talent(roaring_blaze_talent) and target.DebuffRemaining(immolate_debuff) <= BaseDuration(immolate_debuff) and not target.DebuffPresent(roaring_blaze_debuff) and target.TimeToDie() > 10 and { Charges(conflagrate) == 2 + ArmorSetBonus(T19 4) or Charges(conflagrate) >= 1 + ArmorSetBonus(T19 4) and SpellChargeCooldown(conflagrate) < CastTime(immolate) + GCD() or target.TimeToDie() < 24 } and Spell(immolate) or BuffRemaining(conflagration_of_chaos_buff) <= CastTime(chaos_bolt) and Spell(shadowburn) or { Charges(shadowburn) == 1 + ArmorSetBonus(T19 4) and SpellChargeCooldown(shadowburn) < CastTime(chaos_bolt) or Charges(shadowburn) == 2 + ArmorSetBonus(T19 4) } and SoulShards() < 5 and Spell(shadowburn) or Talent(roaring_blaze_talent) and { Charges(conflagrate) == 2 + ArmorSetBonus(T19 4) or Charges(conflagrate) >= 1 + ArmorSetBonus(T19 4) and SpellChargeCooldown(conflagrate) < GCD() or target.TimeToDie() < 24 } and Spell(conflagrate) or Talent(roaring_blaze_talent) and target.DebuffStacks(roaring_blaze_debuff) > 0 and target.DebuffRemaining(immolate_debuff) > target.DebuffDuration(immolate_debuff) * 0.3 and { Enemies(tagged=1) == 1 or SoulShards() < 3 } and SoulShards() < 5 and Spell(conflagrate) or not Talent(roaring_blaze_talent) and BuffStacks(backdraft_buff) < 3 and BuffRemaining(conflagration_of_chaos_buff) <= CastTime(chaos_bolt) and Spell(conflagrate) or not Talent(roaring_blaze_talent) and BuffStacks(backdraft_buff) < 3 and { Charges(conflagrate) == 1 + ArmorSetBonus(T19 4) and SpellChargeCooldown(conflagrate) < CastTime(chaos_bolt) or Charges(conflagrate) == 2 + ArmorSetBonus(T19 4) } and SoulShards() < 5 and Spell(conflagrate) or Talent(empowered_life_tap_talent) and BuffRemaining(empowered_life_tap_buff) <= GCD() and Spell(life_tap)
        {
            ...
            unless Enemies(tagged=1) < 4 and BuffRemaining(active_havoc_buff) > CastTime(chaos_bolt) and Spell(chaos_bolt) or target.DebuffRemaining(immolate_debuff) > CastTime(channel_demonfire) and { Enemies(tagged=1) == 1 or BuffRemaining(active_havoc_buff) < CastTime(chaos_bolt) } and Spell(channel_demonfire) or Enemies(tagged=1) >= 3 and Spell(rain_of_fire) or Enemies(tagged=1) >= 6 and Talent(wreak_havoc_talent) and Spell(rain_of_fire)
            {
...
{
    Enemies(tagged=1) == 2 and Talent(roaring_blaze_talent) and not SpellCooldown(havoc) > 0 and target.DebuffRemaining(immolate_debuff) <= BaseDuration(active_havoc_buff) and Spell(immolate) or { Enemies(tagged=1) < 5 or not Talent(fire_and_brimstone_talent) } and target.DebuffRemaining(immolate_debuff) <= target.TickTime(immolate_debuff) and Spell(immolate) or { Enemies(tagged=1) < 5 or not Talent(fire_and_brimstone_talent) } and { not Talent(cataclysm_talent) or SpellCooldown(cataclysm) >= CastTime(immolate) * Enemies(tagged=1) } and Enemies(tagged=1) > 1 and target.DebuffRemaining(immolate_debuff) <= target.TickTime(immolate_debuff) and { not Talent(roaring_blaze_talent) or not target.DebuffPresent(roaring_blaze_debuff) and Charges(conflagrate) < 2 + ArmorSetBonus(T19 4) } and Spell(immolate) or Talent(roaring_blaze_talent) and target.DebuffRemaining(immolate_debuff) <= BaseDuration(immolate_debuff) and not target.DebuffPresent(roaring_blaze_debuff) and target.TimeToDie() > 10 and { Charges(conflagrate) == 2 + ArmorSetBonus(T19 4) or Charges(conflagrate) >= 1 + ArmorSetBonus(T19 4) and SpellChargeCooldown(conflagrate) < CastTime(immolate) + GCD() or target.TimeToDie() < 24 } and Spell(immolate) or BuffRemaining(conflagration_of_chaos_buff) <= CastTime(chaos_bolt) and Spell(shadowburn) or { Charges(shadowburn) == 1 + ArmorSetBonus(T19 4) and SpellChargeCooldown(shadowburn) < CastTime(chaos_bolt) or Charges(shadowburn) == 2 + ArmorSetBonus(T19 4) } and SoulShards() < 5 and Spell(shadowburn) or Talent(roaring_blaze_talent) and { Charges(conflagrate) == 2 + ArmorSetBonus(T19 4) or Charges(conflagrate) >= 1 + ArmorSetBonus(T19 4) and SpellChargeCooldown(conflagrate) < GCD() or target.TimeToDie() < 24 } and Spell(conflagrate) or Talent(roaring_blaze_talent) and target.DebuffStacks(roaring_blaze_debuff) > 0 and target.DebuffRemaining(immolate_debuff) > target.DebuffDuration(immolate_debuff) * 0.3 and { Enemies(tagged=1) == 1 or SoulShards() < 3 } and SoulShards() < 5 and Spell(conflagrate) or not Talent(roaring_blaze_talent) and BuffStacks(backdraft_buff) < 3 and BuffRemaining(conflagration_of_chaos_buff) <= CastTime(chaos_bolt) and Spell(conflagrate) or not Talent(roaring_blaze_talent) and BuffStacks(backdraft_buff) < 3 and { Charges(conflagrate) == 1 + ArmorSetBonus(T19 4) and SpellChargeCooldown(conflagrate) < CastTime(chaos_bolt) or Charges(conflagrate) == 2 + ArmorSetBonus(T19 4) } and SoulShards() < 5 and Spell(conflagrate) or Talent(empowered_life_tap_talent) and BuffRemaining(empowered_life_tap_buff) <= GCD() and Spell(life_tap) or Enemies(tagged=1) < 4 and BuffRemaining(active_havoc_buff) > CastTime(chaos_bolt) and Spell(chaos_bolt) or target.DebuffRemaining(immolate_debuff) > CastTime(channel_demonfire) and { Enemies(tagged=1) == 1 or BuffRemaining(active_havoc_buff) < CastTime(chaos_bolt) } and Spell(channel_demonfire) or Enemies(tagged=1) >= 3 and Spell(rain_of_fire) or Enemies(tagged=1) >= 6 and Talent(wreak_havoc_talent) and Spell(rain_of_fire) or Talent(empowered_life_tap_talent) and BuffRemaining(empowered_life_tap_buff) < BaseDuration(empowered_life_tap_buff) * 0.3 and Spell(life_tap) or Enemies(tagged=1) < 3 and target.TimeToDie() <= 10 and Spell(chaos_bolt) or Enemies(tagged=1) < 3 and { SpellCooldown(havoc) > 12 and SpellCooldown(havoc) > 0 or Enemies(tagged=1) == 1 or SoulShards() >= 5 - Enemies(tagged=1) * 0.5 } and { SoulShards() >= 5 - Enemies(tagged=1) * 0.5 or BuffRemaining(soul_harvest_buff) > CastTime(chaos_bolt) or BuffRemaining(concordance_of_the_legionfall_buff) > CastTime(chaos_bolt) } and Spell(chaos_bolt) or Enemies(tagged=1) < 3 and { SpellCooldown(havoc) > 12 and SpellCooldown(havoc) > 0 or Enemies(tagged=1) == 1 or SoulShards() >= 5 - Enemies(tagged=1) * 0.5 } and { BuffPresent(trinket_proc_mastery_buff) and BuffRemaining(trinket_proc_mastery_buff) > CastTime(chaos_bolt) or BuffPresent(trinket_proc_crit_buff) and BuffRemaining(trinket_proc_crit_buff) > CastTime(chaos_bolt) or BuffPresent(trinket_proc_versatility_buff) and BuffRemaining(trinket_proc_versatility_buff) > CastTime(chaos_bolt) or BuffPresent(trinket_proc_intellect_buff) and BuffRemaining(trinket_proc_intellect_buff) > CastTime(chaos_bolt) or BuffPresent(trinket_proc_spell_power_buff) and BuffRemaining(trinket_proc_spell_power_buff) > CastTime(chaos_bolt) } and Spell(chaos_bolt) or Enemies(tagged=1) < 3 and { SpellCooldown(havoc) > 12 and SpellCooldown(havoc) > 0 or Enemies(tagged=1) == 1 or SoulShards() >= 5 - Enemies(tagged=1) * 0.5 } and { BuffPresent(trinket_stacking_proc_mastery_buff) and BuffRemaining(trinket_stacking_proc_mastery_buff) > CastTime(chaos_bolt) or BuffPresent(trinket_stacking_proc_crit_buff) and BuffRemaining(trinket_stacking_proc_crit_buff) > CastTime(chaos_bolt) or BuffPresent(trinket_stacking_proc_versatility_buff) and BuffRemaining(trinket_stacking_proc_versatility_buff) > CastTime(chaos_bolt) or BuffPresent(trinket_stacking_proc_intellect_buff) and BuffRemaining(trinket_stacking_proc_intellect_buff) > CastTime(chaos_bolt) or BuffPresent(trinket_stacking_proc_spell_power_buff) and BuffRemaining(trinket_stacking_proc_spell_power_buff) > CastTime(chaos_bolt) } and Spell(chaos_bolt) or Spell(shadowburn) or not Talent(roaring_blaze_talent) and BuffStacks(backdraft_buff) < 3 and Spell(conflagrate) or { Enemies(tagged=1) < 5 or not Talent(fire_and_brimstone_talent) } and { not Talent(cataclysm_talent) or SpellCooldown(cataclysm) >= CastTime(immolate) * Enemies(tagged=1) } and not Talent(roaring_blaze_talent) and target.DebuffRemaining(immolate_debuff) <= BaseDuration(immolate_debuff) * 0.3 and Spell(immolate) or Spell(incinerate) or Spell(life_tap)
}

AddFunction DestructionDefaultCdActions
{
    unless Enemies(tagged=1) == 2 and Talent(roaring_blaze_talent) and not SpellCooldown(havoc) > 0 and target.DebuffRemaining(immolate_debuff) <= BaseDuration(active_havoc_buff) and Spell(immolate) or Enemies(tagged=1) > 1 and { Enemies(tagged=1) < 4 or Talent(wreak_havoc_talent) and Enemies(tagged=1) < 6 } and not target.DebuffPresent(havoc_debuff) and Enemies(tagged=1) > 1 and Spell(havoc text=other) or Charges(dimensional_rift) == 3 and Spell(dimensional_rift) or { Enemies(tagged=1) < 5 or not Talent(fire_and_brimstone_talent) } and target.DebuffRemaining(immolate_debuff) <= target.TickTime(immolate_debuff) and Spell(immolate) or { Enemies(tagged=1) < 5 or not Talent(fire_and_brimstone_talent) } and { not Talent(cataclysm_talent) or SpellCooldown(cataclysm) >= CastTime(immolate) * Enemies(tagged=1) } and Enemies(tagged=1) > 1 and target.DebuffRemaining(immolate_debuff) <= target.TickTime(immolate_debuff) and { not Talent(roaring_blaze_talent) or not target.DebuffPresent(roaring_blaze_debuff) and Charges(conflagrate) < 2 + ArmorSetBonus(T19 4) } and Spell(immolate) or Talent(roaring_blaze_talent) and target.DebuffRemaining(immolate_debuff) <= BaseDuration(immolate_debuff) and not target.DebuffPresent(roaring_blaze_debuff) and target.TimeToDie() > 10 and { Charges(conflagrate) == 2 + ArmorSetBonus(T19 4) or Charges(conflagrate) >= 1 + ArmorSetBonus(T19 4) and SpellChargeCooldown(conflagrate) < CastTime(immolate) + GCD() or target.TimeToDie() < 24 } and Spell(immolate)
    {
        #berserking
        Spell(berserking)
        #blood_fury
        ...
        unless BuffRemaining(conflagration_of_chaos_buff) <= CastTime(chaos_bolt) and Spell(shadowburn) or { Charges(shadowburn) == 1 + ArmorSetBonus(T19 4) and SpellChargeCooldown(shadowburn) < CastTime(chaos_bolt) or Charges(shadowburn) == 2 + ArmorSetBonus(T19 4) } and SoulShards() < 5 and Spell(shadowburn) or Talent(roaring_blaze_talent) and { Charges(conflagrate) == 2 + ArmorSetBonus(T19 4) or Charges(conflagrate) >= 1 + ArmorSetBonus(T19 4) and SpellChargeCooldown(conflagrate) < GCD() or target.TimeToDie() < 24 } and Spell(conflagrate) or Talent(roaring_blaze_talent) and target.DebuffStacks(roaring_blaze_debuff) > 0 and target.DebuffRemaining(immolate_debuff) > target.DebuffDuration(immolate_debuff) * 0.3 and { Enemies(tagged=1) == 1 or SoulShards() < 3 } and SoulShards() < 5 and Spell(conflagrate) or not Talent(roaring_blaze_talent) and BuffStacks(backdraft_buff) < 3 and BuffRemaining(conflagration_of_chaos_buff) <= CastTime(chaos_bolt) and Spell(conflagrate) or not Talent(roaring_blaze_talent) and BuffStacks(backdraft_buff) < 3 and { Charges(conflagrate) == 1 + ArmorSetBonus(T19 4) and SpellChargeCooldown(conflagrate) < CastTime(chaos_bolt) or Charges(conflagrate) == 2 + ArmorSetBonus(T19 4) } and SoulShards() < 5 and Spell(conflagrate) or Talent(empowered_life_tap_talent) and BuffRemaining(empowered_life_tap_buff) <= GCD() and Spell(life_tap) or HasEquippedItem(144369) and not BuffPresent(lessons_of_spacetime_buff) and { not Talent(grimoire_of_supremacy_talent) and not SpellCooldown(summon_doomguard) > 0 or Talent(grimoire_of_service_talent) and not SpellCooldown(service_pet) > 0 or Talent(soul_harvest_talent) and not SpellCooldown(soul_harvest) > 0 } and Spell(dimensional_rift) or Spell(service_imp)
        {
...
{
    Enemies(tagged=1) == 2 and Talent(roaring_blaze_talent) and not SpellCooldown(havoc) > 0 and target.DebuffRemaining(immolate_debuff) <= BaseDuration(active_havoc_buff) and Spell(immolate) or Enemies(tagged=1) > 1 and { Enemies(tagged=1) < 4 or Talent(wreak_havoc_talent) and Enemies(tagged=1) < 6 } and not target.DebuffPresent(havoc_debuff) and Enemies(tagged=1) > 1 and Spell(havoc text=other) or Charges(dimensional_rift) == 3 and Spell(dimensional_rift) or { Enemies(tagged=1) < 5 or not Talent(fire_and_brimstone_talent) } and target.DebuffRemaining(immolate_debuff) <= target.TickTime(immolate_debuff) and Spell(immolate) or { Enemies(tagged=1) < 5 or not Talent(fire_and_brimstone_talent) } and { not Talent(cataclysm_talent) or SpellCooldown(cataclysm) >= CastTime(immolate) * Enemies(tagged=1) } and Enemies(tagged=1) > 1 and target.DebuffRemaining(immolate_debuff) <= target.TickTime(immolate_debuff) and { not Talent(roaring_blaze_talent) or not target.DebuffPresent(roaring_blaze_debuff) and Charges(conflagrate) < 2 + ArmorSetBonus(T19 4) } and Spell(immolate) or Talent(roaring_blaze_talent) and target.DebuffRemaining(immolate_debuff) <= BaseDuration(immolate_debuff) and not target.DebuffPresent(roaring_blaze_debuff) and target.TimeToDie() > 10 and { Charges(conflagrate) == 2 + ArmorSetBonus(T19 4) or Charges(conflagrate) >= 1 + ArmorSetBonus(T19 4) and SpellChargeCooldown(conflagrate) < CastTime(immolate) + GCD() or target.TimeToDie() < 24 } and Spell(immolate) or BuffRemaining(conflagration_of_chaos_buff) <= CastTime(chaos_bolt) and Spell(shadowburn) or { Charges(shadowburn) == 1 + ArmorSetBonus(T19 4) and SpellChargeCooldown(shadowburn) < CastTime(chaos_bolt) or Charges(shadowburn) == 2 + ArmorSetBonus(T19 4) } and SoulShards() < 5 and Spell(shadowburn) or Talent(roaring_blaze_talent) and { Charges(conflagrate) == 2 + ArmorSetBonus(T19 4) or Charges(conflagrate) >= 1 + ArmorSetBonus(T19 4) and SpellChargeCooldown(conflagrate) < GCD() or target.TimeToDie() < 24 } and Spell(conflagrate) or Talent(roaring_blaze_talent) and target.DebuffStacks(roaring_blaze_debuff) > 0 and target.DebuffRemaining(immolate_debuff) > target.DebuffDuration(immolate_debuff) * 0.3 and { Enemies(tagged=1) == 1 or SoulShards() < 3 } and SoulShards() < 5 and Spell(conflagrate) or not Talent(roaring_blaze_talent) and BuffStacks(backdraft_buff) < 3 and BuffRemaining(conflagration_of_chaos_buff) <= CastTime(chaos_bolt) and Spell(conflagrate) or not Talent(roaring_blaze_talent) and BuffStacks(backdraft_buff) < 3 and { Charges(conflagrate) == 1 + ArmorSetBonus(T19 4) and SpellChargeCooldown(conflagrate) < CastTime(chaos_bolt) or Charges(conflagrate) == 2 + ArmorSetBonus(T19 4) } and SoulShards() < 5 and Spell(conflagrate) or Talent(empowered_life_tap_talent) and BuffRemaining(empowered_life_tap_buff) <= GCD() and Spell(life_tap) or HasEquippedItem(144369) and not BuffPresent(lessons_of_spacetime_buff) and { not Talent(grimoire_of_supremacy_talent) and not SpellCooldown(summon_doomguard) > 0 or Talent(grimoire_of_service_talent) and not SpellCooldown(service_pet) > 0 or Talent(soul_harvest_talent) and not SpellCooldown(soul_harvest) > 0 } and Spell(dimensional_rift) or Spell(service_imp) or Enemies(tagged=1) < 4 and BuffRemaining(active_havoc_buff) > CastTime(chaos_bolt) and Spell(chaos_bolt) or target.DebuffRemaining(immolate_debuff) > CastTime(channel_demonfire) and { Enemies(tagged=1) == 1 or BuffRemaining(active_havoc_buff) < CastTime(chaos_bolt) } and Spell(channel_demonfire) or Enemies(tagged=1) >= 3 and Spell(rain_of_fire) or Enemies(tagged=1) >= 6 and Talent(wreak_havoc_talent) and Spell(rain_of_fire) or { target.TimeToDie() <= 32 or not HasEquippedItem(144369) or Charges(dimensional_rift) > 1 or not HasEquippedItem(144369) and { not Talent(grimoire_of_service_talent) or SpellChargeCooldown(dimensional_rift) < SpellCooldown(service_pet) } and { not Talent(soul_harvest_talent) or SpellChargeCooldown(dimensional_rift) < SpellCooldown(soul_harvest) } and { not Talent(grimoire_of_supremacy_talent) or SpellChargeCooldown(dimensional_rift) < SpellCooldown(summon_doomguard) } } and Spell(dimensional_rift) or Talent(empowered_life_tap_talent) and BuffRemaining(empowered_life_tap_buff) < BaseDuration(empowered_life_tap_buff) * 0.3 and Spell(life_tap) or Enemies(tagged=1) < 3 and target.TimeToDie() <= 10 and Spell(chaos_bolt) or Enemies(tagged=1) < 3 and { SpellCooldown(havoc) > 12 and SpellCooldown(havoc) > 0 or Enemies(tagged=1) == 1 or SoulShards() >= 5 - Enemies(tagged=1) * 0.5 } and { SoulShards() >= 5 - Enemies(tagged=1) * 0.5 or BuffRemaining(soul_harvest_buff) > CastTime(chaos_bolt) or BuffRemaining(concordance_of_the_legionfall_buff) > CastTime(chaos_bolt) } and Spell(chaos_bolt) or Enemies(tagged=1) < 3 and { SpellCooldown(havoc) > 12 and SpellCooldown(havoc) > 0 or Enemies(tagged=1) == 1 or SoulShards() >= 5 - Enemies(tagged=1) * 0.5 } and { BuffPresent(trinket_proc_mastery_buff) and BuffRemaining(trinket_proc_mastery_buff) > CastTime(chaos_bolt) or BuffPresent(trinket_proc_crit_buff) and BuffRemaining(trinket_proc_crit_buff) > CastTime(chaos_bolt) or BuffPresent(trinket_proc_versatility_buff) and BuffRemaining(trinket_proc_versatility_buff) > CastTime(chaos_bolt) or BuffPresent(trinket_proc_intellect_buff) and BuffRemaining(trinket_proc_intellect_buff) > CastTime(chaos_bolt) or BuffPresent(trinket_proc_spell_power_buff) and BuffRemaining(trinket_proc_spell_power_buff) > CastTime(chaos_bolt) } and Spell(chaos_bolt) or Enemies(tagged=1) < 3 and { SpellCooldown(havoc) > 12 and SpellCooldown(havoc) > 0 or Enemies(tagged=1) == 1 or SoulShards() >= 5 - Enemies(tagged=1) * 0.5 } and { BuffPresent(trinket_stacking_proc_mastery_buff) and BuffRemaining(trinket_stacking_proc_mastery_buff) > CastTime(chaos_bolt) or BuffPresent(trinket_stacking_proc_crit_buff) and BuffRemaining(trinket_stacking_proc_crit_buff) > CastTime(chaos_bolt) or BuffPresent(trinket_stacking_proc_versatility_buff) and BuffRemaining(trinket_stacking_proc_versatility_buff) > CastTime(chaos_bolt) or BuffPresent(trinket_stacking_proc_intellect_buff) and BuffRemaining(trinket_stacking_proc_intellect_buff) > CastTime(chaos_bolt) or BuffPresent(trinket_stacking_proc_spell_power_buff) and BuffRemaining(trinket_stacking_proc_spell_power_buff) > CastTime(chaos_bolt) } and Spell(chaos_bolt) or Spell(shadowburn) or not Talent(roaring_blaze_talent) and BuffStacks(backdraft_buff) < 3 and Spell(conflagrate) or { Enemies(tagged=1) < 5 or not Talent(fire_and_brimstone_talent) } and { not Talent(cataclysm_talent) or SpellCooldown(cataclysm) >= CastTime(immolate) * Enemies(tagged=1) } and not Talent(roaring_blaze_talent) and target.DebuffRemaining(immolate_debuff) <= BaseDuration(immolate_debuff) * 0.3 and Spell(immolate) or Spell(incinerate) or Spell(life_tap)
}
