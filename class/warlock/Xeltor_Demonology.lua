...
AddFunction no_de2
{
 threemin() and 0 > 0 or threemin() and NotDeDemons(wild_imp) > 0 or threemin() and NotDeDemons(dreadstalker) > 0 or 0 > 0 and NotDeDemons(dreadstalker) > 0 or 0 > 0 and NotDeDemons(wild_imp) > 0 or NotDeDemons(dreadstalker) > 0 and NotDeDemons(wild_imp) > 0 or PreviousGCDSpell(hand_of_guldan) and no_de1()
}
...
AddFunction DemonologyDefaultMainActions
{
 ...
 if DemonDuration(wild_imp) <= ExecuteTime(shadow_bolt) and { BuffPresent(demonic_synergy_buff) or Talent(soul_conduit_talent) or not Talent(soul_conduit_talent) and Enemies(tagged=1) > 1 or Demons(wild_imp) <= 4 } Spell(implosion)
