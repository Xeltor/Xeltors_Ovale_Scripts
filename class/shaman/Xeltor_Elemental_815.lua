...
do
	...
	local desc = "[Xel][8.1.5] Shaman: Elemental"
	local code = [[
...
{
 Talent(totem_mastery_talent_elemental) and TotemRemaining(totem_mastery_elemental) < 2 and { InCombat() or not BuffPresent(ele_resonance_totem_buff) } and Spell(totem_mastery_elemental) or Enemies(tagged=1) > 2 and { Enemies(tagged=1) > 2 or Enemies(tagged=1) > 2 } and ElementalAoeShortCdPostConditions() or ElementalSingleTargetShortCdPostConditions()
}
...
{
 Talent(totem_mastery_talent_elemental) and TotemRemaining(totem_mastery_elemental) < 2 and { InCombat() or not BuffPresent(ele_resonance_totem_buff) } and Spell(totem_mastery_elemental) or Enemies(tagged=1) > 2 and { Enemies(tagged=1) > 2 or Enemies(tagged=1) > 2 } and ElementalAoeCdPostConditions() or ElementalSingleTargetCdPostConditions()
}
...
AddFunction ElementalAoeMainActions
{
 ...
 if target.DebuffRemaining(flame_shock_debuff) < 6 and { Enemies(tagged=1) < 5 - Talent(totem_mastery_talent_elemental no) or not Talent(storm_elemental_talent) and { SpellCooldown(fire_elemental) > 120 + 14 * { 100 / { 100 + SpellCastSpeedPercent() } } or SpellCooldown(fire_elemental) < 24 - 14 * { 100 / { 100 + SpellCastSpeedPercent() } } } } and { not Talent(storm_elemental_talent) or SpellCooldown(storm_elemental) < 120 or Enemies(tagged=1) == 3 and BuffStacks(wind_gust_buff) < 14 } Spell(flame_shock)
...
{
 { InCombat() or not BuffPresent(ele_resonance_totem_buff) } and Spell(totem_mastery_elemental) or Talent(elemental_blast_talent) and Spell(elemental_blast) or not Talent(elemental_blast_talent) and Enemies(tagged=1) < 3 and Spell(lava_burst) or Enemies(tagged=1) > 2 and Spell(chain_lightning_elemental)
}
...
{
 { InCombat() or not BuffPresent(ele_resonance_totem_buff) } and Spell(totem_mastery_elemental) or Talent(stormkeeper_talent) and { 0 < 3 or 600 > 50 } and Spell(stormkeeper) or Talent(elemental_blast_talent) and Spell(elemental_blast) or not Talent(elemental_blast_talent) and Enemies(tagged=1) < 3 and Spell(lava_burst) or Enemies(tagged=1) > 2 and Spell(chain_lightning_elemental)
}
...
AddFunction ElementalSingleTargetMainActions
{
 ...
 if not SpellCooldown(lava_burst) > 0 and Charges(lava_burst) > TalentPoints(echo_of_the_elements_talent) Spell(lava_burst)
 ...
 if BuffPresent(tectonic_thunder_buff) and not BuffPresent(stormkeeper_buff) and Enemies(tagged=1) > 1 Spell(chain_lightning_elemental)
 #lightning_bolt
